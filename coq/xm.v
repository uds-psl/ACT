...
Fact counterexample:
  XM <-> forall A (p: A -> Prop), (forall a, p a) \/ (exists a, ~p a).
Proof.
  split.
  - intros H A p.
    destruct (H (exists a, ~p a)) as [H1|H1].
    + right. exact H1.
    + left. intros a.
      specialize (H (p a)) as [H|H]. exact H.
      exfalso. apply H1. exists a. exact H.
  - intros H P.
...
(** Discrimination lemma for non-repeating lists *)
